...
- (void)retrieveZip:(NSURL *)sourceURL {
    ...
    [request setFailedBlock:^{
        ...
        NSLog(@"Error downloadingzip file: %@", error.localizedDescription);
    }];
...
//    NSString *pattern = @"(href|src)=\"?([\\w:\\/\\.-]*)";
//    NSString *pattern = @"https?://([\\w\\W]*).(jpg|png|gif|jpeg)";
    NSString *pattern = @"[a-zA-z]+://[^\\s]*(jpg|png|jpeg|gif)";
    ...
    [regex enumerateMatchesInString:html options:0 range:NSMakeRange(0, html.length-1) usingBlock:^(NSTextCheckingResult *result, NSMatchingFlags flags, BOOL *stop) {
        ...
        NSString *link = [html substringWithRange:[result rangeAtIndex:0]];
        ...
        if ([linkURL.pathExtension compare:@"zip" options:NSCaseInsensitiveSearch] == NSOrderedSame ||
            ...
            [linkURL.pathExtension compare:@"jpeg" options:NSCaseInsensitiveSearch] == NSOrderedSame ||
            [linkURL.pathExtension compare:@"gif" options:NSCaseInsensitiveSearch] == NSOrderedSame) {                        
...
- (id)initWithHTML:(NSString *)theHtml delegate:(id<ImageManagerDelegate>) theDelegate {
    ...
    if ((self = [super init])) {
        ...
        backgroundQueue = dispatch_queue_create("com.bipin.imageGrabber", nil);
    }
    return self;
}
